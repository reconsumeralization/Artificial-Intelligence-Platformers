...
def parse_args():
    ...
    if not is_root_user(args) and not args.user_install:
        print_error('You need root privileges for the system wide installation process.')
        sys.exit(1)

    if args.user_install and args.destdir == default_user_destdir:
    ...
    if args.system and args.destdir != default_user_destdir:
        print_error(
